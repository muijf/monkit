...
import type { Fiber, ComponentData } from "./types";
import {
  FunctionComponent,
  ClassComponent,
...
export function createTree(currentFiberNode: Fiber): Tree {
  ...
  function _createTree(currentFiberNode: Fiber, tree: Tree): Tree {
      ...
      switch (elementType.name) {
        ...
        case undefined: {
          if (elementType._context?.displayName === "Route") {
            componentData.props = {
              pathname: memoizedProps?.value?.matches?.[0]?.pathname,
            };
            break;
          }
          ...
          if (elementType._context?.displayName === "Location") {
            componentData.props = {
              pathname: memoizedProps?.value?.location?.pathname,
            };
            break;
          }
        }
    ...
    if (
      ...
      componentData.index = masterState.saveNew(stateNode);
    ...
    if (
      (tag === FunctionComponent || tag === IndeterminateComponent) &&
      memoizedState
    ) {
      if (memoizedState.queue) {
        try {
          ...
          hooksStates.forEach(
            ({ state, component, isReducer, lastAction }, i) => {
              componentData.hooksIndex!.push(masterState.saveNew(component));
              ...
              if (isReducer) {
                ...
                (componentData.hooksState as any)[`State: ${i}`] = state;
              }
            }
          );
          newState = {
